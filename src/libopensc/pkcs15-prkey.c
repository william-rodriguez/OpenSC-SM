				 ...
				 const u8 ** buf, size_t *buflen)
{
	...
	if (r == SC_ERROR_ASN1_END_OF_CONTENTS)
		return r;
	SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, r, "PrKey DF ASN.1 decoding failed");
				 ...
				 u8 **buf, size_t *buflen)
{
	...
	for (i=0; i<SC_MAX_SUPPORTED_ALGORITHMS && prkey->algo_refs[i]; i++)   {
		sc_debug(ctx, SC_LOG_DEBUG_ASN1, "Encode algorithm(%i) %i", i, prkey->algo_refs[i]);
		...
		struct sc_pkcs15_object **out_key_object)
{
	...
	SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, rv, "Find private key error");
	...
	sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "CertValue(%i) %p", cert_object->content.len, cert_object->content.value);
	mem = BIO_new_mem_buf(cert_object->content.value, cert_object->content.len);
	if (!mem)
		SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, SC_ERROR_INTERNAL, "MEM buffer allocation error");

	x = d2i_X509_bio(mem, NULL);
	if (!x)
		SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, SC_ERROR_INTERNAL, "x509 parse error");

	buff = OPENSSL_malloc(i2d_X509(x,NULL) + EVP_MAX_MD_SIZE);
	if (!buff)
		SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, SC_ERROR_OUT_OF_MEMORY, "OpenSSL allocation error");
	...
	if (!key_info->subject.value)
		SC_TEST_RET(ctx, SC_LOG_DEBUG_NORMAL, SC_ERROR_OUT_OF_MEMORY, "Subject allocation error");
	...
	sc_debug(ctx, SC_LOG_DEBUG_NORMAL, "Subject %s", sc_dump_hex(key_info->subject.value, key_info->subject.len));
