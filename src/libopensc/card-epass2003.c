...
int epass2003_refresh(struct sc_card *card)
{
	...
	if (g_sm) {
		...
		LOG_TEST_RET(card->ctx, r, "mutural_auth failed");
	}
	return r;
}
...
static int epass2003_sm_free_wrapped_apdu(struct sc_card *card,
...
static int epass2003_sm_get_wrapped_apdu(struct sc_card *card,
		struct sc_apdu *plain, struct sc_apdu **sm_apdu)
{
	struct sc_context *ctx = card->ctx;
